...
public class EnemiesGenerator : Enemy
{
    ...
    public void GenerateEnemy(float heroSpeed)
    {
        ...
        {
            needSpawn = enemyPrefab.IsNeedGenerate(heroSpeed);
            if (needSpawn)
            {
                GameObject newEnemy;

                newEnemy = Instantiate(enemyPrefab.transform.gameObject);
                //newEnemy.name = "Point_" + numberOfEnemy;
                newEnemy.transform.position = cam.transform.position +
                                              new Vector3(0, camCurrentY + ConstantSettings.screenHeightWorld / 2 + 2,
                                                  10);
                lastEnemy = newEnemy;
                numberOfEnemy++;
                break;
    ...
    // public void GenerateEnemyOld(float heroSpeed)
    // {
    //     //camSpeed = heroSpeed;
...
/*
 - Есть массив врагов
 - у каждого врага своя проверка надо ли спавнить этого врага
 - каждый кадр проверяется каждый враг
 - в Генераторе проверяется надо ли спавнить врага и если надо: то цикл проверки прерывается и запускается таймер
        с паузой на возможность генерации следующего врага
 - если надо поставить нового врага, то сначала ищется враг в массиве но уже за пределами экрана
 - если враг в массиве не найден, то спавнится новый экземпляр врага
 */